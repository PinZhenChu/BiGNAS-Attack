...
def search(args):
    ...
    # === 建立 BiGNAS 模型（只建立一次，HardUser & train 共用） ===
    ...
    if args.use_hard_user_augment:
        logging.info("[HardUser] 開始執行方法A（Hard Users + 加邊策略，BiGNAS embedding 版）...")
        ...
        # ✅ 從 BiGNAS 模型抓 user embedding（global id 對應）
        with torch.no_grad():
            user_emb_target = model.embedding.weight[:args.num_users].detach().to(args.device)
        ...
        summary = injector.run(
            ...
            edge_ratio_source=args.edge_ratio_source,
            edge_ratio_target=args.edge_ratio_target,
        )
        ...
        # Debug 冷門商品在 train/valid/test 出現次數
        cold_item_id = args.cold_item_id
        debug_cold_item_counts(split_result, cold_item_id)
        def check_edge_index_global(edge_index, num_nodes, name):
            ...
            if u_min < 0 or u_max >= num_nodes:
                raise ValueError(
                    f"[{name}] user 範圍錯誤: u[{u_min}~{u_max}]，應該在 [0, {num_nodes-1}]"
                )
            if v_min < 0 or v_max >= num_nodes:
                raise ValueError(
                    f"[{name}] item 範圍錯誤: v[{v_min}~{v_max}]，應該在 [0, {num_nodes-1}]"
                )

            logging.info(
                f"[{name}] OK: u[{u_min}~{u_max}], v[{v_min}~{v_max}], num_nodes={num_nodes}"
            )

        # 檢查新增邊是否合法
        num_nodes = args.num_users + args.num_source_items + args.num_target_items
        check_edge_index_global(summary["E_add_source"], num_nodes, "E_add_source")
        check_edge_index_global(summary["E_add_target"], num_nodes, "E_add_target")
